...
Construct your subclass anywhere you'd construct the base adapter — for example, `adapters: { telegram: new CustomTelegramAdapter({ ... }) }`. Members marked `private` (internal caches, in-flight state, secrets) intentionally remain inaccessible; if you find a hook you need that isn't `protected`, please open an issue.
